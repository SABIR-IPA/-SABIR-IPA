...
//  Game
//
//  Created by Harry Dart-O'Flynn on 31/05/2016.
...
var mountNode2 = SCNNode()
var TunnelNode2 = SCNNode()
...
class SecondScene: SCNScene,SCNSceneRendererDelegate,SCNPhysicsContactDelegate {
...
//	var environs = EnvironmentOne(create:true)
	...
	//	rootNode.addChildNode(environs)
    ...
    func followCamera(sender:AnyObject){
        ...
        //  cameraNode.position = SCNVector3(0,25,-4)
        //  cameraNode.eulerAngles = SCNVector3(x: -80, y: 0, z: 0)
        
        cameraNode.position = SCNVector3(x: 0, y: 28, z: -8)
        /// cameraNode.position = SCNVector3(x: 0, y: 10, z: 20)
        
        //  cameraNode.position = SCNVector3(0,20,-45)
    }
...
/*
	func setupLightsAndCamera() {
        ...
        let constraint = SCNLookAtConstraint(target: emptyAtCenter)
		...
		ambiLightNode.light?.color = UIColor.grayColor()
		
		lightNodeDirnl.constraints = [SCNLookAtConstraint(target: emptyAtCenter)]
		
        rootNode.addChildNode(cameraNode)
        rootNode.addChildNode(lightNodeDirnl)
		rootNode.addChildNode(ambiLightNode)
